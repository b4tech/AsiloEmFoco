...
if ($row[0] > 0) {
  ...
  die();
    ...
    if ($query = mysqli_query($connect, "INSERT INTO `idoso` (`nome`, `dataNasc`, `cpf`, `responsavelId`) VALUES ('$nome', '$dataNasc', '$cpf', '$idResponsavel')")) {
        echo "<script language='javascript' type='text/javascript'>alert('Idoso cadastrado com sucesso!');window.location.href='./start/gerenciamentoIdosos.php'</script>";
    } else {
        echo "<script language='javascript' type='text/javascript'>alert('Não foi possível cadastrar esse Idoso.');window.location.href='./start/gerenciamentoIdosos.php'</script>";
    }
}
